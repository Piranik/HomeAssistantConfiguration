...
class Gismeteo:
                ...
                data = {
                    ATTR_FORECAST_TIME:
                        datetime.fromtimestamp(
                            e.get(ATTR_FORECAST_TIME),
                            dt_util.get_time_zone(self._timezone)
                        ).strftime('%FT%T'),
    ...
    @staticmethod
    def _get_datetime(source, tzone):
        local_date = source if len(source) > 10 else source + 'T00:00:00'
        local_date += "+{0:2.2f}".format(
            tzone) if tzone >= 0 else "{0:2.2f}".format(tzone)
        ...
        if self._mode == FORECAST_MODE_HOURLY:
            for day in xml.findall('location/day'):
                ...
                for fc in day.findall('forecast'):
                    ...
                    data = {
                        ...
                        ATTR_FORECAST_TIME:
                            self._get_datetime(fc.get('valid'), tzone),
                        ATTR_FORECAST_CONDITION:
                            fc_v.get('descr'),
                        ATTR_FORECAST_TEMP:
                            int(fc_v.get('t')),
            ...
            for day in xml.findall('location/day[@descr]'):
                data = {
                    ...
                    ATTR_FORECAST_TIME:
                        self._get_datetime(day.get('date'), tzone),
                    ATTR_FORECAST_CONDITION:
                        day.get('descr'),
                    ATTR_FORECAST_TEMP:
                        int(day.get('tmax')),
